...
ALLOWED_ORIGINS: List[str] = os.getenv(
    "AINTRA_CORS", "http://localhost:3000,http://127.0.0.1:3000"
).split(",")
...
# ------ util ops schema untuk FE (opsional, berguna untuk auto-render SettingsPanel) ------
@app.get("/api/operations")
async def list_operations():
    return img_ops.list_operations()
...
async def preview_image(payload: PreviewRequest) -> PreviewResponse:
    try:
    ...
    return PreviewResponse(preview_url=preview_url, metrics=metrics or None)
...
async def process_image(payload: ProcessRequest) -> ProcessResponse:
    ...
    record = await job_manager.submit(stored.path, payload.operation, payload.params)
